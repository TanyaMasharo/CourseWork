// С3_18.cpp: определяет точку входа для консольного приложения.
//
...
//даты, посещаемость занятий, сдача работ
...
struct data{
	...
	Gr variant;//вариант
	Gr control_of_performance;//контроль выполнения
	data *next;
	data *prev;
};
template <class Gr, class fio>
struct fio_s{
	Gr group;//группа
	fio familia[length+1];//фамилия
	fio name[length+1];//имя
	fio otchestvo[length+1];//отчество
	data<Gr, fio> *employment;//занятия
	fio_s *next;
	fio_s *prev;	
};
template <class Gr, class fio>
...
public:	
	...
	//Добавление элементов одновременно с сортировкой
	void add_sort(fio_s<Gr, fio> **mod_beg, fio_s<Gr, fio> **mod_end, fio_s<Gr, fio> * element){
		fio_s<Gr, fio> *pv = new fio_s<Gr, fio>; //добавляемый элемент
		pv = element;
				...
				if ((prim->familia) == (pv->familia)) {
					if ((prim->name) == (pv->name)){
						if ((prim->otchestvo) == (pv->otchestvo)){
							cout << endl << "Такой студент уже существует в списке" << endl; 
						} else goto metka;
					} else goto metka;
	...
	bool remove(fio_s<Gr, fio> **mod_beg, fio_s<Gr, fio> ** mod_end){
		fio_s<Gr, fio> *pv = new fio_s<Gr, fio>;
		cout << " Ввод данных делать на Английском!!!" << endl;
		cout << " Пожалуйста вводите правильно, иначе программа вас не поймет!! " << endl << " Также вводите ФИО с большой буквы!!" << endl << "Вводите ФИО английскими символами!" << endl << endl;
	...
	//---------------------------------------------------------------------------------
	//Работа с датами: Посещаемость
	void attendance(fio_s<Gr, fio> ** mod_beg, fio_s<Gr, fio> ** mod_end){
		system("cls");
		cout << " Пожалуйста вводите правильно, иначе программа вас не поймет!! " << endl << endl;
		cout << " Посещаемость " << endl << endl;
		cout << " Введите дату занятия : " << endl;
		data<Gr, fio> * key = new data<Gr, fio>;
		cout << " День : "; cin >> key->day;
		cout << " Месяц : "; cin >> key->month;
		cout << " Год : "; cin >> key->year;
		fio_s<Gr, fio> * bufer = *mod_beg;
		cout << "Напротив каждой фамилии : 1 - не был(а); 2 - бал(а)" << endl;
		...
		while ((*mod_end)->next) {	
			...
			/*while (((key->presence) != 1) || ((key->presence) != 2)){
				cout << endl << "Вы ввели неправильно! Введите заново : "; cin >> key->presence;
			};*/			
			key->next = 0;
		...
		cout << " Для продолжения нажмите Enter"; _getch();
	};
	//---------------------------------------------------------------------------------
	//Работы с датами: Вывод по дате
	void output_by_data(fio_s<Gr, fio> ** mod_beg, fio_s<Gr, fio> ** mod_end){
		system("cls");
		cout << " Пожалуйста вводите правильно, иначе программа вас не поймет!! " << endl << endl;
		cout << " Вывод по дате " << endl << endl;
		cout << " Введите дату занятия : " << endl;
		data<Gr, fio> * key = new data<Gr, fio>;
		cout << " День : "; cin >> key->day;
		cout << " Месяц : "; cin >> key->month;
		...
		fio_s<Gr, fio> * bufer = *mod_beg;
		while ((*mod_end)->prev != bufer) (*mod_end) = (*mod_end)->prev;
		cout << "Список тех кто явился на занятие : " << endl;
		while ((*mod_end)->next) {	
	...
	//---------------------------------------------------------------------------------
	//Работа с датами: Вывод по номеру лабораторной работы
	void output_by_number(fio_s<Gr, fio> ** mod_beg, fio_s<Gr, fio> ** mod_end){
		system("cls");
		cout << " Пожалуйста вводите правильно, иначе программа вас не поймет!! " << endl << endl;
		cout << " Вывод по номеру лабораторной работы " << endl << endl;
		data<Gr, fio> * key = new data<Gr, fio>;
		cout << " Номер лабораторной работы : "; cin >> key->number;
		fio_s<Gr, fio> * bufer = *mod_beg;
		while ((*mod_end)->prev != bufer) (*mod_end) = (*mod_end)->prev;
		cout << "Список тех у кого введеный номер лабораторной работы : " << endl;
		while ((*mod_end)->next) {	
			data<Gr, fio> * buf = (*mod_end)->employment;
			while (buf) {
	...
	void menu_spisok(){
		while (1){
			system("cls");
			cout << " В данной программе не предусмотренны ошибки пользователя, пожалуйста будьте аккуратны при изменении данных!" << endl;
			cout << " Работа с о списком " << endl << endl;
			switch(menu_spi()){
	...
	//---------------------------------------------------------------------------------
	//Меню работы с датами вывод на экран
	int menu_dat(){
		char bufer[buf];
		int chislo;
		do {
			cout << " 1 - Новое занятие: отметить посещаемость" << endl;
			cout << " 2 - Вывод по дате" << endl;
			cout << " 3 - Вывод по номеру лабораторной работы" << endl;
			cout << " 4 - Выход в главное меню " << endl;
			cin >> bufer;
			chislo = atoi(bufer);
		}while (!chislo);
	...
	//---------------------------------------------------------------------------------
	//Меню общей работы(выбор к датам или списку)
	int menu_work(){
		char bufer[buf];
		int chislo;
		do {
			cout << " 1 - Работать со списком студентов" << endl;
			cout << " 2 - Работать с датами" << endl;
			cout << " 3 - Выход" << endl;
			cin >> bufer;
			chislo = atoi(bufer);
		}while (!chislo);
	...
	void working_with_a_list_and_data(){
		...
		while (1){
			system("cls");
			cout << " В данной программе не предусмотренны ошибки пользователя, пожалуйста будьте аккуратны при изменении данных!" << endl << endl;
			switch(menu_work()){
				case 1: {menu_spisok(); break;};
				case 2: {menu_data(); break;};
				case 3: {return;};
				default: cout << "Надо вводить числа от 1 до 3!!!" << endl; break;
			};
		};	
		filing(element_beg);
	};
...
int main()
{
	setlocale(LC_ALL, "");																	cout << " В данной программе не предусмотренны ошибки пользователя, пожалуйста будте аккуратны при изменении данных!" << endl << endl;
	//---------------------------------------------------------------------------------
